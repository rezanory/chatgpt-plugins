...
def post(endpoint: str, payload: dict | None = None, timeout: int = 180) -> dict:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "v622-kaggle-to-kaggle-transport/1.0",
        },
    )
    try:
...
def download(dest_name: str, path: pathlib.Path) -> dict:
    ...
    for attempt in range(1, 6):
        ...
        if proc.returncode == 0 and path.is_file() and path.stat().st_size > 0:
            ...
            last = (proc.stderr or f"curl exit {proc.returncode}")[-800:]
...
def upload(dest_name: str, path: pathlib.Path) -> dict:
    ...
    for attempt in range(1, 6):
        ...
        last = f"curl={proc.returncode} http={code} stderr={(proc.stderr or '')[-600:]}"
        time.sleep(min(5 * attempt, 20))
...
def main() -> None:
    ...
    for index, row in enumerate(targets, start=1):
        ...
        local = ROOT / dest_name
        print(f"TRANSFER {index}/7 {dest_name}: source download", flush=True)
        ...
        print(f"TRANSFER {index}/7 {dest_name}: uploaded to Kaggle azadka blob", flush=True)
