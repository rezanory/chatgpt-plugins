...
def run_help(argv: list[str], timeout: int = 45) -> tuple[int, str, str]:
    env = {key: value for key, value in os.environ.items() if not SECRET_NAME.search(key)}
    proc = subprocess.run(
        argv,
        check=False,
        capture_output=True,
...
def help_argv(base: list[str], path: list[str]) -> list[str]:
...
def discover(provider: str, depth: int) -> dict:
    ...
    except Exception as exc:  # discovery must survive a version-probe failure
...
def main() -> int:
    ...
    parser.add_argument(
        "--provider",
        choices=["all", "kaggle", "cloudflare", "github"],
        default="all",
    )
