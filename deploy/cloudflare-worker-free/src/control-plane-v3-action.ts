...
async function createM07R320ProducerBridge(env: ActionEnv, body: Rec): Promise<Rec> {
  ...
  const script = [
    ...
    "    with zipfile.ZipFile(payload/name,'r') as z:",
    "        matches=[n for n in z.namelist() if n.endswith(f'fold_{fold_id}/COMPLETED.json')]",
    ...
    "        verified_artifacts={}",
    "        for file_name,expected_sha in (receipt.get('artifact_sha256') or {}).items():",
    ...
    "print('M07_R320_PRODUCER_BRIDGE '+json.dumps({'status':'PASS','producer_ref':CONFIG['producer_ref'],'campaign_receipt_sha256':CONFIG['campaign_receipt_sha256'],'sealed_fold_ids':[1,2,3,4],'missing_fold_ids':[5]},sort_keys=True,separators=(',',':')))",
  ].join("\n");
  ...
  const save = await kaggleAction(
    ...
    {
      slug: bridgeRef,
      newTitle: "M07 R320 Producer Bridge e3884dd1",
      text: script,
      language: "PYTHON",
      kernelType: "SCRIPT",
...
export async function executeKaggleOidcAction(request: Request, env: ActionEnv): Promise<Rec> {
  ...
  const bridgeRequested = service === "m07.RecoveryBridge" && method === "CreateProducerMirror";
  if (
    bridgeRequested &&
    (id !== "kg-05" ||
      opClass !== "compute" ||
      identity.workflow_ref !== "rezanory/chatgpt-plugins/.github/workflows/pneumonia-v17-m07-continuation-20260914.yml@refs/heads/main")
  ) {
    throw new Error("M07 producer bridge is restricted to the trusted main M07 workflow and kg-05 compute recovery");
  }
  if (service.startsWith("m07.") && !bridgeRequested) {
    throw new Error("unsupported M07 recovery broker action");
  }
  const result = bridgeRequested
