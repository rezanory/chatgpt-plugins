...
const EXPECTED_TRANSFORMED_CODE_SHA256 = "9dad6e8bc26f105c7ff21c5aa9050f0878dde9f05cffa14921324b132be1fabe";
...
function normalizedStatus(raw: Record<string, unknown>): string {
  ...
  for (const candidate of [raw.status, raw.statusName, raw.status_name, raw.state, raw.sessionStatus, raw.kernelSessionStatus, session.status, session.statusName, session.state]) {
...
async function saveRepairedKernel(env: Env, sourcePayload: Record<string, unknown>, targetPayload: Record<string, unknown>, text: string): Promise<Record<string, unknown>> {
  const token = env.CGP_KAGGLE_KG05_TOKEN?.trim();
  if (!token) throw new Error("Kaggle kg-05 token missing");
  const sourceMetadata = rec(sourcePayload.metadata);
  const sourceBlob = rec(sourcePayload.blob);
  const targetMetadata = rec(targetPayload.metadata);
  const request: Record<string, unknown> = { slug: TARGET_REF, newTitle: TARGET_TITLE, text, kernelExecutionType: "SAVE_AND_RUN_ALL" };
  if (typeof targetMetadata.id === "number" && targetMetadata.id > 0) request.id = targetMetadata.id;
  copyIfPresent(request, sourceBlob, "language");
  if (request.language === undefined) copyIfPresent(request, sourceMetadata, "language");
  copyIfPresent(request, sourceBlob, "kernelType");
  if (request.kernelType === undefined) copyIfPresent(request, sourceMetadata, "kernelType");
  for (const key of ["datasetDataSources","kernelDataSources","competitionDataSources","categoryIds","isPrivate","enableGpu","enableInternet","dockerImagePinningType","modelDataSources","enableTpu","sessionTimeoutSeconds","priority","dockerImage","machineShape"]) copyIfPresent(request, sourceMetadata, key);
  const response = await fetch(`${API_ROOT}/SaveKernel`, { method: "POST", headers: { Authorization: authHeader(token), "Content-Type": "application/json", "User-Agent": "chatgpt-v622-backbone-repair/1.0" }, body: JSON.stringify(request) });
...
async function repair(env: Env): Promise<Record<string, unknown>> {
  const existing = await listKernels(env, ACCOUNT_ID, TARGET_SLUG, 100);
  const present = existing.some(item => String(rec(item).ref ?? "").toLowerCase() === TARGET_REF.toLowerCase());
  if (!present) throw new Error("Exact target kernel is missing; repair refuses to create a new arbitrary kernel");
  const statusPayload = await kernelStatus(env, ACCOUNT_ID, TARGET_REF);
  const status = normalizedStatus(statusPayload);
  if (status === "COMPLETE" || NON_REPAIRABLE_ACTIVE.has(status)) {
    return { action: "existing", target_kernel_ref: TARGET_REF, status, status_payload: statusPayload, recipe_sha256: RECIPE_SHA256, backbones: BACKBONES, seeds: [42], repair_performed: false };
  }
  if (!REPAIRABLE.has(status)) throw new Error(`Exact target status is not repair-authorized: ${status}`);
  const sourcePayload = await getKernel(env, ACCOUNT_ID, SOURCE_REF);
  const source = typeof rec(sourcePayload.blob).source === "string" ? String(rec(sourcePayload.blob).source) : "";
  if (!source) throw new Error("W16 GetKernel returned no source");
  const targetPayload = await getKernel(env, ACCOUNT_ID, TARGET_REF);
  const text = await transformedNotebook(source);
  const result = await saveRepairedKernel(env, sourcePayload, targetPayload, text);
  return { action: "submitted", target_kernel_ref: TARGET_REF, previous_status: status, result, recipe_sha256: RECIPE_SHA256, source_sha256: EXPECTED_SOURCE_SHA256, transformed_code_sha256: EXPECTED_TRANSFORMED_CODE_SHA256, backbones: BACKBONES, seeds: [42], repair_performed: true, locked_test_used: false, external_data_used: false, canonical_m01_m12_rerun: false, hpo_confirmation_rerun: false };
}

export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    const url = new URL(request.url);
    if (url.pathname === "/healthz" && request.method === "GET") return json({ service: "v622-backbone-repair", status: "ready", protected: true });
    ...
    try { return json({ project: "PNEUMONIA V6.2.2", stage: "post_ablation_backbone_comparison", ...(await repair(env)) }); }
    catch (error) { return json({ project: "PNEUMONIA V6.2.2", stage: "post_ablation_backbone_comparison", action: "blocked", target_kernel_ref: TARGET_REF, recipe_sha256: RECIPE_SHA256, error: error instanceof Error ? error.message.slice(0, 1000) : "unknown error", kaggle_submit_performed: false }); }
  },
