...
manifest={'schema_version':1,'project':'PNEUMONIA V6.2.2','stage':'FINAL_FREEZE_MANIFEST','status':'FROZEN_IN_KAGGLE','master_account':'azadka','source_artifact_dataset':DATASET_REF,'provenance_source_kernel':BACKBONE_REF,'recipe_sha256':RECIPE_SHA,'frozen_policy_file_sha256':POLICY_FILE_SHA,'frozen_policy_semantic_sha256':POLICY_SEMANTIC_SHA,'selected_candidate_ids':SELECTED,'selected_artifact_count':6,'artifacts':rows,'deployment_fit':fit,'qualification_non_consumption_sha256':qsha,'locked_test_used':False,'external_data_used':False,'training_performed':False,'hpo_or_confirmation_repeated':False,'reselection_allowed_after_freeze':False,'next_permitted_stage':'OFFICIAL_LOCKED_TEST'}
...
async function outputReceipt(env:Env):Promise<Rec|null>{
 ...
 if(m.source_artifact_dataset!==DATASET_REF||m.provenance_source_kernel!==BACKBONE_REF||m.recipe_sha256!==RECIPE_SHA256||m.frozen_policy_file_sha256!==POLICY_FILE_SHA256||m.frozen_policy_semantic_sha256!==POLICY_SEMANTIC_SHA256) throw new Error("FINAL_FREEZE canonical identity mismatch");
 if(m.locked_test_used!==false||m.external_data_used!==false) throw new Error("forbidden evidence in FINAL_FREEZE");
 return {manifest_sha256:actual,selected_artifact_count:m.selected_artifact_count,selected_candidate_ids:m.selected_candidate_ids,next_permitted_stage:m.next_permitted_stage,deployment_fit:m.deployment_fit};
}
